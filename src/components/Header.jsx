  ...
  return (
    <header className="header">
      <div className="container flex items-center justify-between">
        {/* logo */}
        <a href="#" className="text-3xl font-semibold"> Kavi-Abey </a>

        {/* mobile menu */}
        <nav className={`lg:hidden absolute top-0 left-0 big-secondary-clr/70 w-full backdrop-blur-2xl max-h-0 overflow-hidden mt-20 flex items-center justify-center transition-[max-height] duration-300 z-20 ${isOpen ? "max-h-[500px] overflow-auto ": ""}`}>
          <ul className="text-canter space-y-6 p-7" >
            {navItems.map((item) => (
              <li key={item.id}>
        ...
        {/* lg menu */}
        <ul className="hidden lg:flex gap-8 iterms-center">
          {navItems.map((item) => (
            <li key={item.id}>
              <a href={item.href} className="text-lg py-3 relative after:absolute after:bottom-0 after:left-0 after:bg-white after:w-0 after:h-0.5 after:rounded hover:after:w-full after:transition-[width] duration-300">{item.label}</a>
            </li>
